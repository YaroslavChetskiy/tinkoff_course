...
public class StatusLogAnalyzer implements LogAnalyzer {
    // SpringFramework нет, поэтому ручками
    private static final Map<Integer, String> REASON_PHRASES = Map.<Integer, String>ofEntries(
    ...
    public Table analyze(List<LogRecord> logs) {
        Table resultTable = new Table("Коды ответа", List.of("Код", "Имя", "Количество"));

        var countMap = logs.stream()
            .collect(groupingBy(LogRecord::status, counting()));
        countMap.entrySet().stream()
            .sorted(Map.Entry.comparingByValue(Comparator.reverseOrder()))
            .forEach(entry -> resultTable.addRow(
                List.of(
                    String.valueOf(entry.getKey()),
                    REASON_PHRASES.get(entry.getKey()),
